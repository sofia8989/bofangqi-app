...
<template>
    <div style="background:#F9F8FC;">
      <div class="recommendBox">
                    ...
                    <div class="hotTitle">同城交友</div>
                    <div class="hotEnglish">Dating Zone</div>
                </div>
          <!-- 列表 -->
          <!-- <ScrollList :loading="state.loading" :finished="state.finished" :refresh="state.refresh" :list="videoList"
            ...
            <div class="px-[10px] pt-[9px] grid grid-cols-2 gap-[5px]">
              <GrilItem v-for="item in state.videoList" :item="item" :key="item.prostituteId" @click="toDetail(item)">
              </GrilItem>
            </div>  
          <!-- </ScrollList> -->
    </div>
  ...
  import { getscFriend } from '@/service/community'
  ...
  // 加载数据
  const init = async () => {
    const listData = await getscFriend({
      // page: state.pageNum,
      // pageSize: 20
    })
