<!-- 轮播图 -->
<template>
    <div style="padding: 0 12px;"  v-if="bannerList.lengtn==0">
        <van-swipe class="my-swipe " style="height: 164px; width: 100%;" :autoplay="3000">
            <van-swipe-item v-for="(item, index) in bannerList" :key="index" @click="$clickAd(item)">
                <img :style="item.img_src=='/assets/default.png' ?'object-fit: cover;':' '" :src="item.img_src"  @error="onImageError(index)" class="swipe-img" />
            </van-swipe-item>
        </van-swipe>
    </div>
</template>
...
const onImageError=(i)=> {
    bannerList.value[i].img_src = '/assets/default.png'
    }
// 轮播图跳转
const goBanner = (item) => {
