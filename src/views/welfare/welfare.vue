...
import { getFreeList } from '@/service/baseApi'
...
const computedVideoList = computed(() => inject_ad_home_swper(state.videoList, xlAVVideoInsertion.value,![69,29].includes(+activeTypeId.value) ? choiceVideoList.value : []))
...
const init = async () => {
  ...
  const res = await getAdultList(params)
  if (params.page == 1) {
